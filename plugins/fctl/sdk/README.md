...
The source repository is private while Reconciliation CI receives a token scoped
to the Reconciliation repository, so a cross-repository credential is the one
thing the gates must not need. Keeping this snapshot in the tree makes them
