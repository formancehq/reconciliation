...
import { useComposedRefs, type PossibleRef } from '@workspace/ui/lib/compose-refs';
...
function StepperList(props: StepperListProps) {
  ...
  const composedRef = useComposedRefs(ref as PossibleRef<HTMLElement>, listRef);
  ...
  const getItems = React.useCallback(
    () =>
      Array.from(itemsRef.current.values()).sort((a, b) => {
...
function StepperTrigger(props: ButtonProps) {
  ...
  const composedRef = useComposedRefs(
    ref as PossibleRef<TriggerElement>,
    setTriggerElement
  );
...
function StepperPrevTrigger(props: ButtonProps) {
  ...
  const PrevTriggerPrimitive = asChild ? SlotPrimitive.Slot : 'button';
...
function StepperNextTrigger(props: ButtonProps) {
  ...
  const NextTriggerPrimitive = asChild ? SlotPrimitive.Slot : 'button';
