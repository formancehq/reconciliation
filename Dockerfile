...
ARG APP_SHA
ARG VERSION
WORKDIR /go/src/github.com/numary/reconciliation
...
RUN --mount=type=cache,id=gomod,target=/go/bridge/mod \
    ...
    go build -o reconciliation \
    -ldflags="-X github.com/numary/reconciliation/cmd.Version=${VERSION} \
    -X github.com/numary/reconciliation/cmd.BuildDate=$(date +%s) \
    -X github.com/numary/reconciliation/cmd.Commit=${APP_SHA}" ./

FROM ubuntu:jammy
RUN apt update && apt install -y ca-certificates && rm -rf /var/lib/apt/lists/*
COPY --from=builder /go/src/github.com/numary/reconciliation/reconciliation /usr/local/bin/reconciliation
EXPOSE 8080
