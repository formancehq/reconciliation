...
| 10 | Transaction-level (lettering) reconciliation — PSP ledger ↔ product ledger per PSP payment reference: flow read with `ListTransactions` filtered on the reference, open holds rewound to the cut through the log window, no query checkpoint; async detail in object storage ([ADR-005](./prd/adr-005-transaction-level-reconciliation.md)) | ⏳ proposed — evaluation done, tickets filed (epic EN-2315; the Ledger V3 migration itself is epic EN-2344) |
