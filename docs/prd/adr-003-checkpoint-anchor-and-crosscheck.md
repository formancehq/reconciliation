...
- **Transaction-level reconciliation ([ADR-005](./adr-005-transaction-level-reconciliation.md), proposed)** — the one read shape this ADR's reasoning did not cover. A per-key extraction spans about N/1000 `ListAccounts` pages, each its own snapshot, so a live read tears. ADR-005 evaluated reintroducing checkpoints for it and measured them at about ×20 slower than live reads. It **keeps this ADR's decision**: the cut is a log id, and a live listing is rewound to it through the log window. Checkpoints serve only as a test oracle, and optionally as a periodic proof that the rewind agrees with a frozen listing.
