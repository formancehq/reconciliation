...
| 1 | 1 | Chart-of-accounts / schema definition (plain Go) | ⬜ next | — |
| 1 | 2 | Bootstrap provisioner (CreateLedger + account-types AUDIT + typed metadata + prepared queries) | ⬜ todo | — |
...
| F6 | LOW | `internal/infra/ledger/` deviates from recon's flat `internal/<domain>` layout (recon has no `infra/`). Mirrors ledger-connect, but inconsistent with host repo. | Decide: keep `infra/ledger` or move to `internal/ledger`. | ⬜ open |
| F7 | LOW | `GetAccount` returns the raw gRPC error (unwrapped) vs wrapping elsewhere. | Wrap for consistency, or accept for thin pass-throughs. | ⬜ open |
