...
Status: **Phase 1 shipped** on `feat/reconciliation-ledger-v3` (P1.1–P1.4: signed writes, end-user attribution, served audit entries and signing keys, verification recipe). Phase 2 (signed closures) needs re-specifying, because the ledger removed chapters ([EN-2351](https://formance-team.atlassian.net/browse/EN-2351)). Tracked in [EN-1930](https://formance-team.atlassian.net/browse/EN-1930). The Postgres implementation ([PR #94](https://github.com/formancehq/reconciliation/pull/94), branch `feat/audit-chain`) is **closed/never merged**.

§§1–5 below are the original review, kept as written. §5's table has been updated to the current state.
...
| Ledger audit entries served by the recon API | ✅ (P1.3) | `GET /audit/entries`, `/audit/entries/{sequence}`, and `/audit/entries/by-transaction/{transactionId}` to resolve a recon object's ledger **tx id** to its entry (`internal/api/audit.go`). Captures, alerts and events carry the tx id, not the `audit_sequence` |
| Token-derived actor attribution | ✅ (P1.2) | the verified token subject is bound into the signed payload for ack / resolve / accept / snooze / unsnooze (`internal/api/service/actor.go`) |
| Period closure / seal / closing write-barrier | ❌ | absent; `periodID` is only an alert scoping key. Phase 2, to re-spec ([EN-2351](https://formance-team.atlassian.net/browse/EN-2351)) |
| External Ed25519 attestation | ✅ (P1.1, P1.4) | every `_recon` write is a `SignedApplyBatch` (`internal/ledger/signing.go`). `GET /audit/signing-keys` serves the keys, and the auditor verifies client-side ([audit-verification.md](audit-verification.md), [verify-audit.mjs](verify-audit.mjs)). There is no `/verify` endpoint, by design |
| Per-alert `/events` | ✅ (W6, half) | a projection over the rule's activity stream (`internal/ledgerstore/alert_events.go`) |
| First-class `/rules/{id}/revisions` | ❌ | revisions can only be reconstructed from the timeline ([EN-2350](https://formance-team.atlassian.net/browse/EN-2350)) |
| Signing-key rotation | ❌ | the first key self-bootstraps; the signed rotation path is not wired ([EN-2352](https://formance-team.atlassian.net/browse/EN-2352)) |
...
- **W6 — Fill the stubs.** The per-alert `/events` endpoint ✅ shipped as an activity-stream projection. `GET /rules/{id}/revisions(/{n})` is still to build ([EN-2350](https://formance-team.atlassian.net/browse/EN-2350)).
