...
| [transaction-level-reconciliation.md](./transaction-level-reconciliation.md) | ⏳ Planned. Transaction-level (lettering) reconciliation of a product ledger against a PSP ledger, per payment id: the booking it relies on (EPHEMERAL holds, indexed key on every lettering transaction), the log-window cut and the *rewind* that makes a live listing exact at the cut without a query checkpoint, what Pebble can and cannot do, and the measured cost of every read path. Decision record: [ADR-005](../prd/adr-005-transaction-level-reconciliation.md). |
