...
| [stale-holds.md](./stale-holds.md) | The `stale_holds` time-based control: where an age signal can come from now that ledger reads are live-only, why the clock stays out of the kernel, and the hold-modelling questions still open with the client. |
