...
pkg/client/                 ✅ Speakeasy-generated public Go SDK (regenerated from openapi.yaml)
plugins/fctl/               ✅ Product-owned fctl command plugin
├── core/catalogue.go        23 commands, exact scopes, Reconciliation major 1 compatibility
├── core/adapter_v1.go       Host-bound generated-client adapter + opaque-cursor pagination
├── component/               Portable component descriptor
├── entrypoints/             WIT describe/start/resume/cancel/close lifecycle
└── audit/                   Reproducible operation inventory and drift checks
```
...
The CLI surface is not wired into `internal/`. It ships as two independent modules:

- **`pkg/client/`** — the public Speakeasy-generated Go SDK, regenerated byte-identically from the repository-root `openapi.yaml`. It owns request and response serialization only; generated authentication, endpoint discovery and retries are all disabled.
- **`plugins/fctl/`** — the product-owned fctl plugin. Its catalogue admits 23 of the 24 operations the contract declares; `GET /_info` is the single exclusion, because the fctl host owns that service-version probe. Endpoint resolution, credentials, transport, retries and rendering stay host-owned, and the plugin injects the host-bound `producthttp` client into the generated SDK.

The plugin pins an exact fctl SDK revision in `plugins/fctl/fctl-sdk.lock.json`, validated by module path, commit, origin, NAR content hash and canonical WIT hash before any plugin command runs. See [plugins/fctl/README.md](../../plugins/fctl/README.md) for the command surface and validation recipes, and [plugins/fctl/docs/command-inventory.md](../../plugins/fctl/docs/command-inventory.md) for the pinned operation inventory and legacy CLI mapping.

Local unit tests and a deterministic component build do not replace install, native-host, browser-host or live-service acceptance.
