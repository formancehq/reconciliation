...
- **`plugins/fctl/`** — the product-owned fctl plugin. Its catalogue admits all 23 public v1 operations; `GET /_info` is excluded because the fctl host owns that service-version probe. Endpoint resolution, credentials, transport, retries and rendering stay host-owned, and the plugin injects the host-bound `producthttp` client into the generated SDK.
