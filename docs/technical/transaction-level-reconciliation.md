...
| F-d | A purged EPHEMERAL account's transactions are no longer returned by an address filter. That includes the **opening** transaction, which was returned before the purge | §2 probe | **L5**: document the contract (reference and metadata stay; the address is lost), decide it together with [EN-2036](https://formance-team.atlassian.net/browse/EN-2036) (implementation in draft PR formancehq/ledger#2058), add a test |
