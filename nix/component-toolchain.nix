...
let
  ...
  componentizeGoVersion = "0.4.1";
  wasiVirtRevision = "448f6df8f688cee5d6995e96b1ffc31f9bf00742";
  ...
  componentize-go = rustPlatform.buildRustPackage {
    ...
    version = componentizeGoVersion;
    src = pkgs.fetchFromGitHub {
      owner = "bytecodealliance";
      repo = "componentize-go";
      rev = "v${componentizeGoVersion}";
  ...
  wasi-virt = rustPlatform.buildRustPackage {
    pname = "wasi-virt";
    version = "0.2.0-${builtins.substring 0 8 wasiVirtRevision}";
    src = pkgs.fetchFromGitHub {
      owner = "bytecodealliance";
      repo = "WASI-Virt";
      rev = wasiVirtRevision;
  ...
  binaryen = assert pkgs.binaryen.version == "124"; pkgs.binaryen;
in
{
  inherit componentize-go wasi-virt wasm-tools binaryen;
}
