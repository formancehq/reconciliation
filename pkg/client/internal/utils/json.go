...
func UnmarshalJSON(b []byte, v interface{}, tag reflect.StructTag, topLevel bool, requiredFields []string) error {
	...
	case isModelType(typ):
		if bytes.Equal(b, []byte("null")) {
			return unmarshalJSONLossless(b, v)
		}
		...
		if err := unmarshalJSONLossless(b, &unmarshaled); err != nil {
			return err
		}
...
func unmarshalValue(value json.RawMessage, v reflect.Value, tag reflect.StructTag) error {
	if bytes.Equal(value, []byte("null")) {
		if v.CanAddr() {
			return unmarshalJSONLossless(value, v.Addr().Interface())
		} else {
			return unmarshalJSONLossless(value, v.Interface())
		}
	}
	...
	case reflect.Int64:
		...
		if format == "string" {
			var s string
			if err := unmarshalJSONLossless(value, &s); err != nil {
				return err
			}
	...
	case reflect.Map:
		...
		if err := unmarshalJSONLossless(value, &unmarshaled); err != nil {
			return err
		}
	...
	case reflect.Struct:
		...
		case reflect.TypeOf(big.Int{}):
			...
			if format == "string" {
				...
				if err := unmarshalJSONLossless(value, &b); err != nil {
					return err
				}
			}
	...
	return unmarshalJSONLossless(value, val)
}
