...
func ReconciliatePayouts(ctx context.Context, db *mongo.Database, pspIdPath string) error {
	...
	reconciliations, err := UnmarshalPayout(ctx, paymentCursor)

	for _, recon := range reconciliations {
		status, err := rules.ReconciliationPayout(ctx, recon)
		if err != nil {
			return err
		}

		fullTxs, err := UpdatePayoutStatus(ctx, db, recon, *status)
		if err != nil {
			return err
		}
...
func UnmarshalPayout(ctx context.Context, paymentCursor *mongo.Cursor) ([]storage.PaymentReconciliation, error) {
...
func UpdatePayoutStatus(ctx context.Context, db *mongo.Database, agg storage.PaymentReconciliation, status storage.ReconciliationStatus) ([]storage.FullReconTransaction, error) {

	var payouts []storage.FullReconTransaction
	...
	// update txledger
	//TODO: we could maybe do an updateMany with the psp_id filter and type = payout ?
	for _, tx := range agg.Transactions {
		...
		payouts = append(payouts, storage.FullReconTransaction{
			Transaction:          tx,
			ReconciliationStatus: storage.Statuses{"payout": status},
		})
	}
